            ...
            <div class="d-none col-md-6" id="p{{$d->id}}">
                        ...
                        <form action="{{url('vote/'.$d->id.'/'.$d->user_id)}}">
                            ...
                            <button type="submit" class="btn">
                                <i id="{{'love'.$d->id}}" class="fas fa-heart fa-2x" style="position:relative; right: 0 !important;"></i>
                            </button>
                                <ul>Likes : <b>{{$d->likes}}</b></ul>
                                <ul>Nama : {{$d->user->name}}</ul>   
                                <ul>Asal Sekolah : {{$d->user->participant->first()->sekolah}}</ul>
                        </form>
                </div>
                @endforeach
            </div>
...
<footer class="p-2 text-white" style="background-color: rgb(89,89,89); margin-top:330px;">
    ...
    <script>
        ...
        nama.addEventListener('click', e => {
            var jumbotron = document.querySelector("#p"+e.target.id);
            jumbotron.classList.toggle("d-none");
            jumbotron.classList.toggle("muncul");
        });
        ...
        var expireTime = waktu + 1000*60*60*24*14;
        now.setTime(expireTime);
        const love = document.querySelector(".poster");
        love.addEventListener("click", e => {
            if (getCookie(e.target.id) == "sudah vote") {
                document.cookie = e.target.id + ' = cancel vote; expires = ' + now.toUTCString();
            } else {
                document.cookie = e.target.id + ' = sudah vote; expires = ' + now.toUTCString();
            }
            console.log(e.target.id);
        })
        let id = [{{implode(",", $id)}}];

        for (let i = 0; i <= Math.max.apply(Math, id); i++) {
            if(getCookie('love'+i) == "sudah vote") {
                var hati = document.querySelector("#love"+i);
                hati.style.color = "red";
            }
